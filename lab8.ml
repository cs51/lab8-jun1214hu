...
......................................................................*)
  ...
  let add_listener (evt : 'a event) (listener : 'a -> unit) : id =
    (* create waiter from listener and give it unique ID  and add to event *)
    let id_num = new_id () in
    let waiter = {id = id_num; action = listener} in
    evt := waiter :: !evt ;
    id_num ;;
...
let receive_report (s : string) : unit =
  ignore (WEvent.addlistener publish (fun () -> fakeNewsNetwork s));
  ignore (WEvent.addlistener publish (fun () -> buzzFake s));;
...
WEvent.addlistener newswire recieve_report ;;
